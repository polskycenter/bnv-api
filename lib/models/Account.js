...
//      see beforeRestore below as well for restoration workflow
Account.hook('beforeDestroy', (account) => {
  const email = `${encodeURIComponent(account.email)}@${Date.now().toString(36)}.bnv.io`;

  return account.update({ email });
});

Account.hook('beforeFind', (options) => {
...
Account.hook('afterRestore', (account) => {
