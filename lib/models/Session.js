...
function compareSessionTimestamp(a, b) {
  return timestamp(b.timeUsed || b.timeCreated) - timestamp(a.timeUsed || a.timeCreated);
}
...
Session.prune = function prune(accountId) {
  return this.findAll({
    ...
    .then(sessions => sessions.sort(compareSessionTimestamp))
    .then(sessions => sessions.slice(2).map(session => session.destroy()));
};
